...
resource "azurerm_linux_virtual_machine" "vm1" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "cd ~/sas_viya_playbook",
      "sudo tee /etc/systemd/system/setupSasWorkOnStartup.service /dev/null << \"EOF\"",
      "[Unit]",
      ...
      "sudo tee /etc/systemd/scripts/setupSasWorkOnStartup.sh /dev/null << \"EOF\"",
      "#!/bin/bash",
      ...
      "SAS_USER=\"sas\"",
      "SAS_GROUP=\"sas\"",
      "while ! mountpoint -q /mnt; do sleep 1; echo \"Waiting on /mnt to mount...\"; done",
      "echo \"creating saswork\"",
      "mkdir -p /mnt/saswork",
      "echo \"creating cascache\"",
      "mkdir -p /mnt/cascache",
      "echo \"granting rights to created temp dir\"",
      "chown -R ${SAS_USER}:${SAS_GROUP} /mnt/cascache",
      "chown -R ${SAS_USER}:${SAS_GROUP} /mnt/saswork",
      "echo \"opening group permissions so the cas can write as well\"",
      "chmod -R 777 /mnt/saswork",
      "chmod -R 777 /mnt/cascache",
      "EOF",
      "sudo chmod 700 /etc/systemd/scripts/setupSasWorkOnStartup.sh",
