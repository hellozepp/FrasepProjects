...
/* 07SEP2018   FRANIR      Added CASLIB name, CAS Table name,        */
/*                         profile run data to every output table to */
/*                         enable history                            */
/*                         Moved from a CASL data step to a SAS data */
/*                         step                                      */
/*                         Added append, index and save capabilities */
/* ***************************************************************** */
...
%let inputProfileTable=profile_table ; /* the profiling result table */
%let inCaslib=public ; /* the caslib where this table is */

%let outCaslib=public ; /* the caslib where you want to create the profiling data mart */
%let append_master=yes ; /* yes/no if you want to append the results - set no the first time to create/overwrite the tables */

cas mySession sessopts=(caslib="&inCaslib") ;

libname profsrc cas caslib="&inCaslib" ;
libname profdm cas caslib="&inCaslib" ;

proc cas;
dataDiscovery.profile result=r1 / table={caslib="&inCaslib" name="HMEQ_TRAIN"}, casOut={name="&inputProfileTable"};
run;
quit;

/* Save the table information to a dataset */
...
/* The creation date of the profiling result table is the profiling run date */
...
/* Get global metrics to broadcast them in all output tables */
...
data profdm.tmp_profdm_column_metrics
   ...
   */
    ...
    /* FRANIR - added job_run_date table_source_caslib_name table_source_table_name to all output tables */
    /* Also added table_nrows to COLUMN_METRICS table for computing NON NULL COUNT in the VA report */
    job_run_date=&profileJobDateTime ;
    table_source_caslib_name=strip("&table_source_caslib_name") ;
    table_source_table_name=strip("&table_source_table_name") ;
...
/* Now (9.4M5 or Viya SPRE) %if is accepted in open code outside of a macro */
%if %upcase(&append_master=NO) %then %do ;

    /* No append so drop target tables */
...
/* Append to the existing (or non-existing if overwrite) tables */
...
/* Clean temporary session tables */

proc casutil incaslib="&outCaslib" ;
    list tables ;
    droptable casdata="tmp_profdm_column_metrics" quiet ;
    droptable casdata="tmp_profdm_tableinfo" quiet ;
    droptable casdata="tmp_profdm_frequencies_high" quiet ;
    droptable casdata="tmp_profdm_frequencies_low" quiet ;
    droptable casdata="tmp_profdm_patterns_high" quiet ;
    droptable casdata="tmp_profdm_patterns_low" quiet ;
    droptable casdata="tmp_profdm_idanalysis" quiet ;
    droptable casdata="tmp_profdm_outliers_high" quiet ;
    droptable casdata="tmp_profdm_outliers_low" quiet ;
    list tables ;
run ;
quit ;

/* Index the tables on the primary columns used for filters in the VA report */

proc cas ;
    table.index / table={caslib="&outCaslib" name="profdm_tableinfo"} casout={caslib="&outCaslib" name="idx_profdm_tableinfo" indexVars={"table_source_caslib_name","table_source_table_name","job_run_date"}} ;
    table.index / table={caslib="&outCaslib" name="profdm_column_metrics"} casout={caslib="&outCaslib" name="idx_profdm_column_metrics" indexVars={"table_source_caslib_name","table_source_table_name","job_run_date"}} ;
    table.index / table={caslib="&outCaslib" name="profdm_frequencies_high"} casout={caslib="&outCaslib" name="idx_profdm_frequencies_high" indexVars={"table_source_caslib_name","table_source_table_name","job_run_date"}} ;
    table.index / table={caslib="&outCaslib" name="profdm_frequencies_low"} casout={caslib="&outCaslib" name="idx_profdm_frequencies_low" indexVars={"table_source_caslib_name","table_source_table_name","job_run_date"}} ;
    table.index / table={caslib="&outCaslib" name="profdm_patterns_high"} casout={caslib="&outCaslib" name="idx_profdm_patterns_high" indexVars={"table_source_caslib_name","table_source_table_name","job_run_date"}} ;
    table.index / table={caslib="&outCaslib" name="profdm_patterns_low"} casout={caslib="&outCaslib" name="idx_profdm_patterns_low" indexVars={"table_source_caslib_name","table_source_table_name","job_run_date"}} ;
    table.index / table={caslib="&outCaslib" name="profdm_idanalysis"} casout={caslib="&outCaslib" name="idx_profdm_idanalysis" indexVars={"table_source_caslib_name","table_source_table_name","job_run_date"}} ;
    table.index / table={caslib="&outCaslib" name="profdm_outliers_high"} casout={caslib="&outCaslib" name="idx_profdm_outliers_high" indexVars={"table_source_caslib_name","table_source_table_name","job_run_date"}} ;
    table.index / table={caslib="&outCaslib" name="profdm_outliers_low"} casout={caslib="&outCaslib" name="idx_profdm_outliers_low" indexVars={"table_source_caslib_name","table_source_table_name","job_run_date"}} ;
quit ;
run ;
...
proc casutil incaslib="&outCaslib" outcaslib="&outCaslib" ;
    list files ;
    save casdata="profdm_tableinfo" casout="profdm_tableinfo_&SYSDATE9" replace ;
    save casdata="profdm_column_metrics" casout="profdm_column_metrics_&SYSDATE9" replace ;
    save casdata="profdm_frequencies_high" casout="profdm_frequencies_high_&SYSDATE9" replace ;
    save casdata="profdm_frequencies_low" casout="profdm_frequencies_low_&SYSDATE9" replace ;
    save casdata="profdm_patterns_high" casout="profdm_patterns_high_&SYSDATE9" replace ;
    save casdata="profdm_patterns_low" casout="profdm_patterns_low_&SYSDATE9" replace ;
    save casdata="profdm_idanalysis" casout="profdm_idanalysis_&SYSDATE9" replace ;
    save casdata="profdm_outliers_high" casout="profdm_outliers_high_&SYSDATE9" replace ;
    save casdata="profdm_outliers_low" casout="profdm_outliers_low_&SYSDATE9" replace ;
    list files ;
run ;
quit ;
