options cashost='frasepviya35smp.cloud.com' casport=5570 casdatalimit=ALL;

cas datagen01 sessopts=(timeout=3600);
caslib _all_ assign;

%macro generate(n_rows=10, n_num_cols=1, n_char_cols=1, mindate='01jan2005'd, maxdate='30jun2009'd, outdata=test, seed=0);
	data &outdata;
		array nums[&n_num_cols];
		array chars[&n_char_cols] $30;
		char_length=8;
		temp="";
		range=&maxdate-&mindate+1;
		format range date date9.;

		do i=1 to &n_rows;
			do j=1 to &n_num_cols;
				nums[j]=ranuni(&seed);
			end;

			do j=1 to &n_char_cols;
				char_length=ceil(ranuni(&seed)*100);
				chars[j]=substr(temp, ceil(ranuni(&seed)*1100), char_length);
			end;
			date=&mindate + int(ranuni(&seed)*range);
			output;
		end;
		drop i j temp range char_length;
	run;
%mend generate;

%generate(n_rows=100000,n_num_cols=5,n_char_cols=5,outdata=casuser.test_data1,seed=0);
/* Size = 	20,800,000 bytes */

%generate(n_rows=1000000,n_num_cols=5,n_char_cols=5,outdata=casuser.test_data2,seed=0);
/* Size = 	208,000,000 bytes */

%generate(n_rows=10000000,n_num_cols=5,n_char_cols=5,outdata=casuser.test_data3,seed=0);
/* Size = 2,080,000,000 bytes */

proc cas;
	table.tabledetails / caslib="casuser" name="test_data1";
	table.tabledetails / caslib="casuser" name="test_data2";
	table.tabledetails / caslib="casuser" name="test_data3";

	table.promote / sourcecaslib="casuser" name="test_data1" target="test_data1" targetcaslib="public" drop=true;
	table.promote / sourcecaslib="casuser" name="test_data2" target="test_data2" targetcaslib="public" drop=true;
	table.promote / sourcecaslib="casuser" name="test_data3" target="test_data3" targetcaslib="public" drop=true;
quit;


cas datagen01 terminate;
